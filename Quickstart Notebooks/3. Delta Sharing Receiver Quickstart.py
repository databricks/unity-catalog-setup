...
# MAGIC %md
# MAGIC 
# MAGIC ### Scratch work below 

# COMMAND ----------

# MAGIC %md
# MAGIC ## Add Share to Secret Scope
# MAGIC 
# MAGIC For security it is advisable to store the share file in the Databricks secret scope. 
# MAGIC 
# MAGIC This can be done using the databricks-cli package. <br>
# MAGIC We can use it to help us create a secret scope and add a new secret to the scope.

# COMMAND ----------

# MAGIC %md
# MAGIC For databricks-cli to work correctly it requires a .databrickscfg file to be populated. <br>
# MAGIC .databrickscfg should at least contain \[DEFAULT] scope. <br>
# MAGIC In default scope you should specify the databricks host and the api token. <br>
# MAGIC This will enable databricks-cli to issue commands.
...
profile_file_scope = "delta_sharing_scope"
os.system(f"databricks secrets create-scope --scope {profile_file_scope}")


# COMMAND ----------

# MAGIC %sh wget https://databricks-datasets-oregon.s3-us-west-2.amazonaws.com/delta-sharing/share/open-datasets.share

# COMMAND ----------

file_path = "open-datasets.share"
key = "deltashare.quickstart.profile"
os.system(f"databricks secrets put --scope {profile_file_scope} --key {key} --binary-file {file_path}")

# COMMAND ----------

# MAGIC %sh databricks secrets list --scope  delta_sharing_scope --output JSON
